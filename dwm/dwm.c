...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - (int)TEXTW(stext) - getsystraywidth(m))
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
monocle(Monitor *m)
{
	...
	if (n > 0) /* override layout symbol */ {
		...
		char size[12] = "";
		sprintf(size, "%d", n);
		strcat(newStr, layouts[2].symbol);
		strcat(newStr, size);
		snprintf(m->ltsymbol, sizeof m->ltsymbol, "%s", newStr);
	}
